...
- すべてのユーザー認証が確実にオンプレミスで行われる
- この方法では、管理者はより厳しいレベルのアクセス制御（たとえば、スマートカード ベースの認証やサードパーティの多要素認証）を実装することができる
- Azure Active Directory (Azure AD) Connect を使用して、オンプレミスの Active Directory フェデレーション サービス (AD FS) と Azure AD でフェデレーションを構成
...
プロビジョニング システム以外のオンプレミスのサーバーの要件: Azure AD Connect | なし | 追加の**証エージェント**ごとに 1 つのサーバー | 2 つ以上の AD FS サーバー境界/DMZ ネットワークに 2 つ以上の WAP サーバー
